...
def g(x):
    return torch.exp(-50 * (x)**2) # Defines and sets the initial condition g(x) with a gaussian/normal distribution "bump" to see how the wave propagates

u = torch.zeros((nx, nt))        # Initializing the solution tensor u with zeros 
u[:, 0] = g(x)                   # Initial condition at t = 0
...
class InitialConditionPrediction(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.model = nn.Sequential(
            ...
            # Output layer to predict initial condition g
        )
...
def train_model(model, inputs, ground_truth_u, epochs, lr):
    ...
    for epoch in range(epochs):
        ...
        loss = loss_function(predicted_u, ground_truth_u) #outputs should be predicted_u
        pinn_loss_history.append(loss.item())  # Store loss for plotting later
        optim.zero_grad()
        #adjoint_gradients = adjoint_method(ground_truth_u=ground_truth_u, u=outputs.unsqueeze(1))  # Calculate gradients using adjoint method
        loss.backward()
